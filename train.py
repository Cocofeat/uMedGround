...
def parse_args(args):
    parser = argparse.ArgumentParser(description="uMedGround Model Training")
    parser.add_argument("--local_rank", default=0, type=int, help="node rank")
    parser.add_argument(
        # "--version", default="xinlai/LISA-7B-v1"
        # "--version", default="xinlai/LISA-13B-llama2-v1"
        # "--version", default="/root/autodl-tmp/cache/hub/models--liuhaotian--llava-llama-2-13b-chat-lightning-preview"
    ...
    parser.add_argument("--lora_r", default=8, type=int) # 8/16
    ...
    parser.add_argument("--train_data_type", default="None", type=str) # /None/CheXpert
    ...
    parser.add_argument(
        "--batch_size", default=2, type=int, help="batch size per device per step: 2/4/8"
    )
    parser.add_argument(
    ...
    parser.add_argument("--reg_loss_weight", default=1.0, type=float) # 0.5/1
    ...
    parser.add_argument("--lora_alpha", default=16, type=int) # 8/16
    parser.add_argument("--lora_dropout", default=0.05, type=float)
    parser.add_argument("--lora_target_modules", default="q_proj,v_proj", type=str)
    parser.add_argument("--explanatory", default=0.1, type=float) # -1
    ...
    parser.add_argument("--vision_pretrained", default="./pretrained/sam_vit_h_4b8939.pth", type=str) # SAM
    # parser.add_argument("--vision_pretrained", default="./pretrained/medsam_vit_b.pth", type=str) # MedSAM
    ...
    parser.add_argument("--llava_sam_head", action="store_true", default=True)
...
def train(
    ...
    box_reg_llava_losses = AverageMeter("BoxRegLlavaLoss", ":.4f")
    box_reg_mhp_losses = AverageMeter("BoxRegMhpLoss", ":.4f")
    box_evidential_losses = AverageMeter("BoxEvidentialLoss", ":.4f")
    box_uncertain_box_losses = AverageMeter("BoxUncertainBoxLoss", ":.4f")
    ...
    for global_step in range(args.steps_per_epoch):
        for i in range(args.grad_accumulation_steps):
            ...
            if args.uncertain_box_head:
                box_uncertain_box_loss = output_dict["box_uncertain_box_loss"]
            elif args.mhp_box_head:
                box_reg_mhp_loss = output_dict["box_mhp_box_loss"]
            elif args.llava_sam_head:
                box_reg_llava_loss = output_dict["box_reg_llava_loss"]
            ...
            if args.uncertain_box_head:
                ...
                box_reg_mhp_losses.update(box_reg_mhp_loss.item(), input_dict["images"].size(0))
            elif args.llava_sam_head:
                box_reg_llava_losses.update(box_reg_llava_loss.item(), input_dict["images"].size(0))
        ...
        if global_step % args.print_freq == 0:
            if args.distributed:
                ...
                if args.uncertain_box_head:
                    ...
                    box_reg_mhp_losses.all_reduce()
                elif args.llava_sam_head:
                    box_reg_llava_losses.all_reduce()
            ...
            if args.local_rank == 0:
                ...
                if args.uncertain_box_head:
                    ...
                    writer.add_scalar(
                    "train/box_reg_mhp_loss", box_reg_mhp_losses.avg, global_step
                )
                elif args.llava_sam_head:
                    writer.add_scalar(
                    "train/box_reg_llava_loss", box_reg_llava_losses.avg, global_step
                )
                writer.add_scalar(
            ...
            if args.uncertain_box_head:
                ...
                box_reg_mhp_losses.reset()
            elif args.llava_sam_head:
                box_reg_llava_losses.reset()
...
def validate(val_loader, model_engine, epoch, writer, args):
    ...
    for input_dict in tqdm.tqdm(val_loader):
        ...
        intersection, acc = torch.mean(miou).cpu().numpy(), torch.mean(accu).cpu().numpy()
